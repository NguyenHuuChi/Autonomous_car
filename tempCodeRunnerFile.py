...
def birdview_transform(img):
    ...
    src = np.float32([[0, IMAGE_H*2//3-20], [320, IMAGE_H*2//3-20], [
                     0, 0], [IMAGE_W, 0]])
    dst = np.float32([[90, IMAGE_H], [265, IMAGE_H],[-100, 0], [IMAGE_W+150, 0],])
...
def find_lane_lines(image,left_point_list,right_point_list,draw=False):
    ...
    interested_line_y2=int(im_height*0.755)
    if draw:
    ...
    if left_point1==-1:
        if abs(right_point1-left_point_list[-1])<20:
            ...
            check_steering=1
        elif left_point2!=1  and right_point2!=1:
            left_point1=right_point1-lane_width
        else:
            a=(left_point_list[-1]-left_point_list[0])//2
            left_point1=left_point_list[-1]+a
